...
# Pick a sensible worker count for your machine
workers <- 6
...
# Optional: avoid too many Arrow threads per worker (keeps I/O civilized)
# We'll set it at the top of each worker.
process_year_parallel <- function(yr, weights_dt) {
  # throttle Arrow CPU threads per worker (adjust to taste)
  try({ arrow::set_cpu_count(1L) }, silent = TRUE)
  tryCatch(
    {
      process_year(yr, weights_dt)
      sprintf("Year %d: ok", yr)
    },
    error = function(e) sprintf("Year %d: ERROR - %s", yr, conditionMessage(e))
  )
}


# Kick it off
jj::timed("start (parallel)")
msgs <- future_lapply(
  years_all,
  function(yr) process_year_parallel(yr, weights_dt),
  future.seed = TRUE
)
jj::timed("end (parallel)")
